...
class SearchEngine:
    ...
    def get_all_cocktails(self) -> list:
        ...
        # command = f'SELECT * from cocktails ORDER BY base, name'
        command = f'SELECT * FROM cocktails WHERE id >= 1 AND id <=10'
        return self.d_db.sql_command(command)
    
    def get_all_bases(self) -> list:
        ''' Returns all bases in database '''
        # command = f'SELECT DISTINCT base from cocktails ORDER BY base'
        command = f'SELECT DISTINCT base from cocktails WHERE id >= 1 AND id <=10 ORDER BY base'
        return self.d_db.sql_command(command)
